...
#define KACCAKMAGIC_LEN (sizeof(keccakmagic) - 1)
#define KECCAK_SERIALIZATION_LEN                                                     \
    (                                                                                \
        KACCAKMAGIC_LEN /* magic string */                                           \
    ...
    unsigned char *output, size_t *outlen)
{
    ...
    /* Magic code */
    memcpy(p, keccakmagic, KACCAKMAGIC_LEN);
    p += KACCAKMAGIC_LEN;
    ...
    const unsigned char *input, size_t len)
{
    ...
    /* Magic code */
    if (memcmp(input, keccakmagic, KACCAKMAGIC_LEN) != 0)
        return 0;

    p = input + KACCAKMAGIC_LEN;
