...
int ossl_lib_ctx_attach_conf_imodule(OSSL_LIB_CTX *ctx, CONF_IMODULE *md)
{
    if (ctx == NULL || md == NULL || ctx->imod != NULL)
        return 0;
...
int ossl_lib_ctx_detach_conf_imodule(OSSL_LIB_CTX *ctx, CONF_IMODULE *md)
{
    if (ctx != NULL && md != NULL)
        return 0;

    if (ctx != NULL && ctx->imod) {
        ctx->imod->libctx = NULL;
        ctx->imod = NULL;
    }
...
static int context_deinit(OSSL_LIB_CTX *ctx)
{
    ...
    ossl_lib_ctx_detach_conf_imodule(ctx, NULL);
...
void *ossl_lib_ctx_get_data(OSSL_LIB_CTX *ctx, int index)
{
    ...
    case OSSL_LIB_CTX_CONF_IMODULE:
        return (void *)ctx->imod;
