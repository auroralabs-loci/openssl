 ...
 *
 * The Extensible Authentication Protocol (EAP) based Flexible Authentication
 * via Secure Tunneling (EAP-FAST) protocol. EAP-FAST is an EAP method that
...
static const char eap_psk_secret[] = "shared-secret";
static const char eap_psk_identity[] = "identity";

static const unsigned char eap_sid_req[] = {
    0xde, 0xad, 0xbe, 0xef, 0x01, 0x02, 0x03, 0x04,
    0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c
};

static const unsigned char eap_sid_res[] = {
    0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
    0xde, 0xad, 0xbe, 0xef, 0x01, 0x02, 0x03, 0x04
};

static const struct eap_ciphersuites {
    char *name;
} eap_css[] = {
...
{
    if (max < (sizeof(eap_psk_secret) - 1))
        return 0;
    memcpy(psk, eap_psk_secret, (sizeof(eap_psk_secret) - 1));
    return (unsigned int)(sizeof(eap_psk_secret) - 1);
}
...
static unsigned int client_psk_cb(SSL *ssl, const char *hint,
    ...
    unsigned char *psk, unsigned int max)
{
    if (max < (sizeof(eap_psk_secret) - 1) || max_id < sizeof(eap_psk_identity))
        return 0;
    strncpy(identity, eap_psk_identity, max_id);
    memcpy(psk, eap_psk_secret, (sizeof(eap_psk_secret) - 1));
    return (unsigned int)(sizeof(eap_psk_secret) - 1);
}
...
static int eap_ctx_set_cache(SSL_CTX *s_ctx, SSL_CTX *c_ctx)
{
...
static int eap_ctx_set_ticket(SSL_CTX *s_ctx, SSL_CTX *c_ctx)
{
    SSL_CTX_set_psk_server_callback(s_ctx, server_psk_cb);
    SSL_CTX_set_psk_client_callback(c_ctx, client_psk_cb);
    SSL_CTX_set_session_cache_mode(s_ctx, SSL_SESS_CACHE_SERVER);
...
static int eap_set_shutdown(SSL *c, SSL *s)
{
...
static int eap_set_server_cache(SSL_CTX *s_ctx)
{
...
static int eap_set_callbacks(SSL *c, SSL *s)
{
...
static int eap_ctx_enforce_tls12(SSL_CTX *s_ctx, SSL_CTX *c_ctx)
{
    int test;
...
static int eap_sessid_matches(SSL *c, SSL *s)
{
...
/*
 * https://github.com/openssl/openssl/issues/29095
 * Regression test for the following bug (EAP-FAST / TLS 1.2):
 *
 * On the first connection, TLS 1.2 PSK performs a full handshake and
 * produces a session object with a session ID. That session can be cached
 * server-side and resumed on subsequent connections. OpenSSL handles this
 * transparently: if SSL_CTX_sess_set_get_cb() returns a valid SSL_SESSION*
 * whose cipher suite is PSK-based, the server proceeds with an abbreviated
 * handshake. The PSK identity and key are already embedded in the cached
 * session, there is nothing left to verify. When a TLS 1.2 PSK session is
 * resumed via an external server-side cache SSL_CTX_sess_set_get_cb(), the PSK
 * callback is not invoked and the cached session is trusted directly.
 *
 ...
 * a full PSK handshake. Catching the mismatch here ensures the server never
...
{
    const struct eap_ciphersuites *cs = &eap_css[idx];
...
static int test_tls12_psk_resume_ticket_mismatch(int idx)
{
    ...
    test = TEST_ptr(s_ctx = SSL_CTX_new(TLS_server_method()))
        ...
        && TEST_true(SSL_CTX_set_cipher_list(s_ctx, cs->name))
        && TEST_true(SSL_CTX_set_cipher_list(c_ctx, cs->name))
        && TEST_true(eap_ctx_set_ticket(s_ctx, c_ctx))
        && TEST_true(create_ssl_objects(s_ctx, c_ctx, &s, &c, NULL, NULL))
        && TEST_true(eap_set_callbacks(c, s))
        && TEST_true(create_ssl_connection(s, c, SSL_ERROR_NONE))
        ...
        && TEST_int_eq(eap_set_shutdown(c, s), 1)
        && TEST_ptr(r_sess = dup_session_with_id(c_sess, eap_sid_req, sizeof(eap_sid_req)))
        && TEST_ptr(q_sess = dup_session_with_id(s_sess, eap_sid_res, sizeof(eap_sid_res)))
        && TEST_true(create_ssl_objects(s_ctx, c_ctx, &s_ssl, &c_ssl, NULL, NULL))
        && TEST_true(eap_set_callbacks(c_ssl, s_ssl))
        && TEST_true(SSL_set_session(c_ssl, r_sess))
        ...
        && TEST_true(create_ssl_connection(s_ssl, c_ssl, SSL_ERROR_NONE))
        && TEST_true(SSL_session_reused(s_ssl))
        && TEST_true(eap_sessid_matches(c_ssl, s_ssl));
...
#ifndef OPENSSL_NO_PSK
    ADD_ALL_TESTS(test_tls12_psk_resume_sessid_mismatch, OSSL_NELEM(eap_css));
    ADD_ALL_TESTS(test_tls12_psk_resume_ticket_mismatch, OSSL_NELEM(eap_css));
#endif
    return 1;
}
