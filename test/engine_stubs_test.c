...
static int test_engine_stubs(void)
{
    ...
    if (!TEST_ptr_null(ENGINE_get_RAND(NULL)))
        return 0;
    if (ENGINE_get_destroy_function(NULL) != (ENGINE_GEN_INT_FUNC_PTR)NULL)
        return 0;
    if (ENGINE_get_init_function(NULL) != (ENGINE_GEN_INT_FUNC_PTR)NULL)
        return 0;
    if (ENGINE_get_finish_function(NULL) != (ENGINE_GEN_INT_FUNC_PTR)NULL)
        return 0;
    if (ENGINE_get_ctrl_function(NULL) != (ENGINE_CTRL_FUNC_PTR)NULL)
        return 0;
    if (ENGINE_get_load_privkey_function(NULL) != (ENGINE_LOAD_KEY_PTR)NULL)
        return 0;
    if (ENGINE_get_load_pubkey_function(NULL) != (ENGINE_LOAD_KEY_PTR)NULL)
        return 0;
    if (ENGINE_get_ssl_client_cert_function(NULL) != (ENGINE_SSL_CLIENT_CERT_PTR)NULL)
        return 0;
    if (ENGINE_get_ciphers(NULL) != (ENGINE_CIPHERS_PTR)NULL)
        return 0;
    if (ENGINE_get_digests(NULL) != (ENGINE_DIGESTS_PTR)NULL)
        return 0;
    if (ENGINE_get_pkey_meths(NULL) != (ENGINE_PKEY_METHS_PTR)NULL)
        return 0;
    if (ENGINE_get_pkey_asn1_meths(NULL) != (ENGINE_PKEY_ASN1_METHS_PTR)NULL)
        return 0;
