 ...
 * when X509_add_cert() or sk_X509_CRL_push() fails. We run the same
 * code path under OPENSSL_MALLOC_FAILURES so that when the failure hits
 * the add/push path, the fix ensures the cert/CRL is freed and we do not leak.
 * When run under valgrind (OSSL_USE_VALGRIND=yes), a leak would be reported.
 */
