...
identifier.
The B<issuer> keyword asks that the AKID include both the issuer's directory
name and its serial number, by default as a fallback when no keyid is available.
...
if that element is cannot be included.
