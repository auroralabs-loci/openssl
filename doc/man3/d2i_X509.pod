...
Otherwise, a pointer to a B<I<TYPE>> object is returned and I<*ppin> is
incremented to the byte following the parsed data.
The caller retains ownership of the returned object and needs to free it when
it is no longer needed, e.g. via X509_free() for X509 objects or
DSA_SIG_free() for DSA_SIG objects.

If either I<a> or else I<*a> is NULL fresh storage is allocated for the
returned object, and if I<a> is not NULL then I<*a> is set equal to the
returned pointer.

Finally, when both I<a> and I<*a> are not NULL, I<*a> MUST be a pointer to an
