...
a fresh standalone connection returned by L<SSL_new(3)>, using a method such
as L<OSSL_QUIC_method(3)> or L<OSSL_QUIC_client_method(3)>. It must not have
been started or shut down, have streams or network BIOs configured on it, or
belong to another QUIC object hierarchy.

On success, I<new_conn> is adopted into the listener's object hierarchy and
uses its shared network port and listener-derived TLS configuration.
Certificates, verification parameters, ciphersuites, ALPN configuration and
other context-level security policy must be configured on the listener's
B<SSL_CTX>. Applications must not install a private connected BIO or file
descriptor on the returned connection. Thread-assisted operation configured
on I<new_conn> is not retained.
...
L<SSL_accept(3)> must not be used for this purpose, as it is not supported by
the QUIC client methods.
